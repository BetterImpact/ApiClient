...
{
    /// <summary>
    /// Worked examples of the jobs people most often want to automate against the enterprise api. Each one takes a
    /// filter so you decide which people it runs for (including which organizations they belong to), and each one
    /// writes what it is doing to the console.
    /// </summary>
    static class Examples
    {
        ...
        /// user.
        /// </summary>
        /// <param name="whichUsers">
        /// Decides who is in the group, for example every accepted volunteer of a particular set of organizations.
        /// </param>
        public static void DownloadCustomFieldFilesForAGroupOfUsers(Client client, UsersFilterModelEnterprise whichUsers, string destinationFolder)
        {
        ...
        /// </summary>
        public static void DownloadTimeClockQRCodes(Client client, UsersFilterModelEnterprise whichUsers, string destinationFolder)
        {
        ...
        /// asking for the timelog entries of those specific people. Because this is the enterprise api the hours can
        /// come from more than one organization, so the organizations they came from are counted as well.
        /// </summary>
        /// <param name="organizationIds">
        /// Optional. When supplied, only hours worked at these organizations are counted. Pass the same ids on
        /// whichUsers if you also want the list of people narrowed down to those organizations.
        /// </param>
        public static void ListUsersAndTheirHours(Client client, UsersFilterModelEnterprise whichUsers, DateTime workedFrom, DateTime workedTo, List<int> organizationIds)
        {
            //the memberships carry the running hours total the system keeps for each volunteer at each organization,
            //which is worth having next to the total we are about to add up ourselves.
            ...
            var organizationsWorkedAt = users.ToDictionary(user => user.UserId, user => new HashSet<int>());

            //rather than pulling every timelog entry in the enterprise and throwing most of them away, ask for the
            ...
            {
                var whichEntries = new TimelogFilterModelEnterprise
                {
                ...
                foreach (var entry in EnumerateTimelogEntries(client, whichEntries))
                {
                    ...
                    organizationsWorkedAt[entry.UserId].Add(entry.OrganizationId);
                }
            }
        ...
        /// </remarks>
        private static IEnumerable<User> EnumerateUsers(Client client, UsersFilterModelEnterprise whichUsers)
        {
            ...
            while (true)
            {
                var page = client.ListEnterpriseUsers(whichUsers);
        ...
        /// </summary>
        private static IEnumerable<Timelog> EnumerateTimelogEntries(Client client, TimelogFilterModelEnterprise whichEntries)
        {
            ...
            while (true)
            {
                var page = client.ListEnterpriseTimelogEntries(whichEntries);
